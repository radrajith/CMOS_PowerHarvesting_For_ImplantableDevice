function [] = powercalc(file, header, repRate, no_row, no_col,startpt, endpt)
coilV = (2-1)*repRate*2;        %for power of 1 mosfet we subtract coiv -rect
vrect = (3-1)*repRate*2;
cur = (8-1)*repRate*2;
...
for x = 1: 2: repRate*2
    ...
    %avgvolt = mean(voltage(startpt:endpt));        %for avg power calc
    %avgcurr = mean(current(startpt:endpt));        %for avg power calc
